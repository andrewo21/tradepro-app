...
app.post("/api/ai/extract-summary", async (req, res) => {
  try {
    ...
    const completion = await client.chat.completions.create({
      model: "gpt-4o",
      messages: [{ role: "system", content: "Professional resume writer. Create a 3-4 sentence construction summary. Return ONLY text." }, { role: "user", content: text }],
      temperature: 0.5,
    });
...
app.post("/api/ai/rewrite", async (req, res) => {
  try {
    ...
    const completion = await client.chat.completions.create({
      model: "gpt-4o",
      messages: [{ role: "system", content: "Elite Construction Recruiter. Fix grammar and translate to professional English. Return ONLY text." }, { role: "user", content: `Rewrite this ${type}: ${text}` }],
      temperature: 0.3,
    });
...
// --- 2. THE MASTER TEMPLATE REGISTRY ---
...
const templateRegistry = {
  ...
  "basic-two-column": (doc, data) => {
    ...
    doc.font("Helvetica").fontSize(10).text(data.summary || "", sidebarWidth + 40, 75, { width: 330 });
    (data.experience || []).forEach(job => {
        doc.moveDown(1).font("Helvetica-Bold").text(`${job.jobTitle} — ${job.company}`, sidebarWidth + 40);
        const bullets = [...(job.responsibilities || []), ...(job.achievements || [])];
        bullets.forEach(r => {
            const txt = typeof r === 'string' ? r : (r.text || "");
            if (txt) doc.font("Helvetica").text(`• ${txt}`, sidebarWidth + 50);
        });
    });
  },
  ...
  "modern-blue": (doc, data) => {
    ...
    doc.fillColor("black").moveDown(5).font("Helvetica-Bold").fontSize(14).text("Experience", 40);
    (data.experience || []).forEach(job => {
      doc.moveDown(1).font("Helvetica-Bold").text(`${job.jobTitle} — ${job.company}`);
      const bullets = [...(job.responsibilities || []), ...(job.achievements || [])];
      bullets.forEach(r => {
        const txt = typeof r === 'string' ? r : (r.text || "");
        if (txt) doc.font("Helvetica").fontSize(10).text(`• ${txt}`, 50);
      });
    });
  },
  ...
  "standard-classic": (doc, data) => {
    ...
    doc.fillColor("black").moveDown(4).font("Helvetica-Bold").fontSize(11).text("EXPERIENCE");
    (data.experience || []).forEach(job => {
      doc.moveDown(1).font("Helvetica-Bold").text(`${job.jobTitle} — ${job.company}`);
      const bullets = [...(job.responsibilities || []), ...(job.achievements || [])];
      bullets.forEach(r => {
  ...
  "executive-luxe": (doc, data) => {
    ...
    doc.font("Helvetica-Bold").fontSize(11).text("EXPERIENCE", sidebarWidth + 30, 50);
    (data.experience || []).forEach(job => {
        doc.moveDown(1).font("Helvetica-Bold").text(`${job.jobTitle} — ${job.company}`, sidebarWidth + 30);
        const bullets = [...(job.responsibilities || []), ...(job.achievements || [])];
        bullets.forEach(r => {
          const txt = typeof r === 'string' ? r : (r.text || "");
          if (txt) doc.font("Helvetica").text(`• ${txt}`, sidebarWidth + 40);
        });
    });
  },
